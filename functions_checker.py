...
PYTA_CONFIG = 'functions_pythonta.json'
TARGET_LEN = 79
SEP = '='
...
class TestChecker:
    ...
    def test_get_hypertension_data(self) -> None:
        ...
        with open('hypertension_data_small.csv') as f:
            d = {}
            assert functions.get_hypertension_data(d, f) is None, \
    ...
    def test_get_low_income_data(self) -> None:
        ...
        with open('low_income_small.csv') as f:
            d = {}
            assert functions.get_low_income_data(d, f) is None, \
    ...
    def test_get_bigger_neighbourhood(self) -> None:
        ...
        self._check(functions.get_bigger_neighbourhood,
                    [SAMPLE_DATA,
                     'Mount Olive-Silverstone-Jamestown',
                     'Rexdale-Kipling'],
                    str)
    ...
    def test_get_high_hypertension_rate(self) -> None:
        ...
        results = checker_generic.returns_list_of_Ts(
            functions.get_high_hypertension_rate, [SAMPLE_DATA, 0.24], tuple)
        assert results[0], results[1]

    def test_get_ht_to_low_income_ratios(self) -> None:
        """Function get_ht_to_low_income_ratios"""
        self._check(functions.get_ht_to_low_income_ratios,
                    [SAMPLE_DATA], dict)

    def test_calculate_ht_rates_by_age_group(self) -> None:
        """Function calculate_ht_rates_by_age_group"""
        results = checker_generic.returns_tuple_of(functions.calculate_ht_rates_by_age_group,
                                                   [SAMPLE_DATA,
                                                    'Elms-Old Rexdale'],
                                                   (float, float, float))
        assert results[0], results[1]
    ...
    def test_get_stats_summary(self) -> None:
        ...
        self._check(functions.get_stats_summary, [SAMPLE_DATA], float)

    def test_order_by_ht_rate(self) -> None:
        """Function order_by_ht_rate"""
        results = checker_generic.returns_list_of_Ts(functions.order_by_ht_rate,
                                                     [SAMPLE_DATA], str)
        assert results[0], results[1]
    ...
    def test_check_constants(self) -> None:
        ...
        self._check_constants(CONSTANTS, functions)
        print('  check complete')

    def test_test_functions_has_test_cases(self) -> None:
        """test_functions.py has test cases beyond the provided 'test_first_bigger'
        """
        import test_functions
        test_cases = [item for item in dir(test_functions)
                      if item.startswith('test_')
                      and item != 'test_first_bigger']
        assert test_cases, \
            "No new test cases were found in test_functions.py: " \
